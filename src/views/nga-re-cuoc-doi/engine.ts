...
export class GameEngine {
    ...
    getNextEvent(): DATA.GameEvent | null {
        ...
        const milestone = DATA.MILESTONE_EVENTS.find(e => e.triggerAge === this.state?.age && !this.usedEvents.has(e.id));
        if (milestone) {
            event = { ...milestone };
        } else {
            const pool = [];
            ...
            if (DATA.DANGEROUS_EVENTS && Math.random() < 0.10) {
    ...
    applyChoice(choice: DATA.Choice) {
        ...
        if (this.state?.activeBuffs && this.state.activeBuffs.find((b: { type: string }) => b.type === 'double_positive')) {
        ...
        for (const [key, value] of Object.entries(resolvedEffects)) {
            if (this.state.stats[key] !== undefined) {
                this.state.stats[key] = Math.max(0, Math.min(100, this.state.stats[key] + (value as number)));
            }
        }
    ...
    _getFallbackEvent(): DATA.GameEvent {
        return {
            ...
            choices: [
                { text: 'Làm việc chăm chỉ', icon: '💪', hint: 'Tích lũy kỹ năng', effects: { skill: 5, money: 3, stress: 3 }, result: 'Một năm làm việc hiệu quả!' },
                { text: 'Tận hưởng cuộc sống', icon: '🎶', hint: 'Vui vẻ, thư giãn', effects: { happiness: 8, stress: -5, relationships: 5 }, result: 'Năm nay bạn sống rất vui!' }
            ]
        };
    }
    ...
    _applyPassiveEffects() {
        ...
        if (this.state.stats.stress > 70) this.state.stats.health = Math.max(0, this.state.stats.health - 3);
        if (this.state.stats.health < 30) this.state.stats.happiness = Math.max(0, this.state.stats.happiness - 3);
    ...
    _checkGameOver() {
        ...
        if (this.state.age >= this.MAX_AGE) {
            this.state.gameOver = true;
            return { reason: 'age', message: 'Bạn đã đến tuổi nghỉ hưu!' };
        }
        if (this.state.stats.health <= 0) {
            this.state.gameOver = true;
            return { reason: 'health', message: 'Sức khỏe của bạn kiệt quệ...' };
        }
        if (this.state.stats.happiness <= 0 && this.state.stats.stress >= 100) {
            this.state.gameOver = true;
            return { reason: 'burnout', message: 'Bạn hoàn toàn kiệt sức và mất niềm vui sống...' };
        }
        return null;
    }
    ...
    calculateEnding() {
        ...
        if (total >= 350) { title = 'Cuộc Đời Viên Mãn'; icon = '🏆'; subtitle = 'Bạn đã sống một cuộc đời trọn vẹn, cân bằng và hạnh phúc!'; }
        else if (total >= 280) { title = 'Cuộc Đời Thành Công'; icon = '⭐'; subtitle = 'Bạn đạt được nhiều thành tựu đáng tự hào!'; }
        else if (total >= 200) { title = 'Cuộc Đời Bình Dị'; icon = '🌿'; subtitle = 'Cuộc sống bình thường nhưng ý nghĩa theo cách riêng.'; }
        else if (total >= 120) { title = 'Cuộc Đời Gập Ghềnh'; icon = '🌧️'; subtitle = 'Nhiều khó khăn nhưng bạn vẫn vượt qua.'; }
        else { title = 'Cuộc Đời Đầy Thử Thách'; icon = '💔'; subtitle = 'Cuộc sống không dễ dàng, nhưng bạn đã cố gắng.'; }
    ...
    _updateSkillCooldowns() {
        ...
        if (this.state?.activeBuffs) {
            this.state.activeBuffs = this.state.activeBuffs.filter((buff: { type: string, duration: number }) => {
                buff.duration--;
    ...
    _isEventImmune(eventId: string) {
        ...
        if (this.state.activeBuffs) {
            const immunityBuff = this.state.activeBuffs.find((b: { type: string }) => b.type === 'immunity_danger');
    ...
    saveGame() {
        if (!this.state) return false;
        localStorage.setItem('ngare_save', JSON.stringify({ state: this.state, used: [...this.usedEvents] }));
        return true;
    }
    ...
    buyItem(itemId: string) {
        ...
        if (this.state.inventory.includes(itemId)) return { success: false, message: 'Bạn đã sở hữu vật phẩm này' };
    ...
    buySkill(skillId: string) {
        ...
        if (this.state.activeSkills.includes(skillId)) return { success: false, message: 'Bạn đã mở khóa kỹ năng này' };
    ...
    useSkill(skillId: string) {
        ...
        if (!skill) return { success: false, message: 'Kỹ năng không tồn tại' };
        if (!this.state.activeSkills.includes(skillId)) return { success: false, message: 'Bạn chưa mở khóa kỹ năng này' };
        ...
        const result: { message?: string, previewEvent?: DATA.GameEvent | null, skill?: DATA.ActiveSkill, isPreview?: boolean } = {};
        switch(skill.effect) {
            case 'reset_stress':
