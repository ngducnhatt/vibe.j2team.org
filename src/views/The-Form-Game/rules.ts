...
const numWords = ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'một', 'hai', 'ba', 'bốn', 'năm', 'sáu', 'bảy', 'tám', 'chín', 'mười']
...
export const rules: Rule[] = [
    ...
    },
    {
        ...
        validator: (f) => {
            const langs = ['python', 'javascript', 'java', 'c++', 'typescript', 'c#', 'ruby', 'php', 'swift', 'go', 'kotlin', 'rust', 'c', 'sql', 'cpp', 'cs']
    ...
    {
        id: 11,
        description: 'Mật khẩu KHÔNG được chứa chữ số dạng số (VD: 0, 1, 2...). Hãy dùng chữ cái (VD: một, hai, one, two...). Ngoại lệ: chỉ cho phép dãy số của hôm nay (DDMM) nếu có yêu cầu.',
        validator: (f) => {
    ...
    {
        id: 12,
        description: 'Độ dài vòng lặp: Tổng số lượng CHỮ CÁI trong Mật khẩu phải bằng chính xác GẤP ĐÔI độ dài Username.',
        validator: (f) => {
    ...
    {
        id: 17,
        description: 'Bio phải nhắc tên một ngôn ngữ lập trình không có thật (UltraScript, QuantumJS, HyperLang...). Hãy xoá nguyên âm: ltrscrpt, qntmjs, hyprlng.',
        validator: (f) => {
    ...
    {
        id: 19,
        description: 'Mật khẩu phải mô tả Username (chứa các tính từ như fast, slow, hot, big, bad, good, quick, smart).',
        validator: (f) => {
    ...
    {
        id: 20,
        description: 'AI duyệt form: form này phải làm AI thấy hợp lý. Username chứa từ hài hước (funny/haha), mật khẩu nên thơ (poetic), hoặc bio kỳ cục (wrd / weird).',
        validator: (f) => {
            const u = f.username.toLowerCase()
            ...
            return u.includes('funny') || u.includes('haha') || p.includes('poetic') || b.split(/[\s,.'"]+/).includes('wrd') || b.includes('krpy')
        }
    }
]
