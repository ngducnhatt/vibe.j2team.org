<template>
  <div class="flex flex-col items-center w-full">
    <p class="text-text-primary font-display font-medium text-lg mb-6 text-center leading-relaxed">
      Nhấn nút dưới đây <br/> <span class="text-accent-sky font-bold text-xl">CHÍNH XÁC sau 3 giây</span> kể từ khi bắt đầu.
    </p>
    ...
    <div class="relative w-full max-w-sm flex flex-col items-center">
      ...
      <div v-if="state === 'failed'" class="mt-6 text-center animate-fade-in-up">
        ...
        <p class="text-text-secondary text-sm mt-1">Đã trôi qua: {{ (timeElapsed / 1000).toFixed(3) }} giây.</p>
      ...
      <div v-if="state === 'success'" class="mt-6 text-center animate-bounce-in">
        ...
        <p class="text-text-primary mt-1">Chuẩn {{ (timeElapsed / 1000).toFixed(3) }}s. Bạn như một cái đồng hồ đo.</p>
      </div>
    </div>
  </div>
</template>
