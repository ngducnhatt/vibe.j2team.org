<template>
  <div class="min-h-screen bg-bg-deep text-text-primary p-4 md:p-8 font-sans selection:bg-accent-coral selection:text-text-primary flex flex-col items-center">
    ...
    <!-- Header -->
    <header class="max-w-4xl w-full mb-10 border-b border-border-default pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
      <div>
        ...
        <p class="text-sm text-text-secondary mt-3 font-medium border-l-2 border-accent-sky pl-3 max-w-sm">
          Hệ thống cần xác nhận bạn là một thực thể sinh học.
        </p>
      </div>
      <router-link to="/" class="group flex items-center gap-2 text-sm font-display font-semibold text-text-secondary hover:text-accent-sky transition-colors duration-300">
    ...
    <main class="max-w-2xl w-full flex-1 pb-16 flex flex-col items-center">
      ...
      <div v-if="store.fullyVerified" class="w-full text-center mt-10 animate-bounce-in">
        <div class="w-20 h-20 bg-emerald-500/20 text-emerald-500 rounded-none flex items-center justify-center mx-auto mb-6 transform -rotate-6 border border-emerald-500 shadow-[4px_4px_0_0_#10b981]">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
          </svg>
        </div>
        <h2 class="text-3xl font-display font-bold text-text-primary mb-4 tracking-tight">Xác thực thành công.</h2>
        <p class="text-lg text-text-secondary">Chúc mừng. Bạn đã chính thức được công nhận là con người.</p>
        
        <router-link to="/" class="inline-block mt-8 bg-bg-deep border border-border-default text-text-primary px-8 py-3 font-display font-bold transition-all hover:bg-border-default">
          Quay lại sảnh chính
        </router-link>
      </div>
      ...
      <div v-else class="w-full">
        ...
        <div class="mb-8 w-full flex flex-col items-center text-center">
          <div class="flex gap-2 mb-2">
            <div 
              ...
              :class="index < store.currentChallengeIndex ? 'bg-accent-sky' : (index === store.currentChallengeIndex ? 'bg-accent-sky/50 animate-pulse' : 'bg-border-default')"
            ></div>
          </div>
        ...
        <!-- Troll Message -->
         <div v-if="showTrollMessage" class="w-full text-center bg-accent-coral/10 border border-accent-coral p-6 mb-8 animate-shake">
            <p class="text-accent-coral font-bold font-display text-lg uppercase tracking-wider mb-2">Kiểm tra lại chút...</p>
            <p class="text-text-primary text-sm">Chỉ để cho chắc chắn thôi. Làm lại nhé.</p>
         </div>

        <!-- Challenge Container -->
        <div class="w-full bg-bg-surface rounded-none border border-border-default p-6 md:p-10 relative transition-all duration-300 shadow-[8px_8px_0_0_rgba(255,255,255,0.05)] min-h-[300px] flex items-center justify-center overflow-hidden">
    ...
    <!-- Footer Credit -->
    <footer class="w-full max-w-4xl border-t border-border-default pt-6 pb-2 text-left flex justify-between items-center px-2">
